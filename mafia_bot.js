...
const LEAGUES = [
  ...
  { id: 'AURORA',  emoji: '🌈', name: 'لیگ شفقِ پفیِ افسانه‌ای', requiredScore: 12000 } // خیلی خوشگل
];
...
function createRoom(roomId) {
  ...
  const room = {
    ...
    status: 'WAITING',
    waiting: [],
    players: [], // { userId, nickname, role, alive, stats, jailed }
    day: 0,
...
function goToCourt(room, targetId) {
  ...
  broadcastToRoom(
    ...
    '💬 چت: فقط برای متهم (در این نسخه، فقط به‌صورت ذهنی!)'
  );
...
function handleHelp(msg) {
  ...
  bot.sendMessage(
    ...
    '۲️⃣ دکمه «شروع بازی آنلاین» → ورود به صف اتاق\n' +
    '۳️⃣ روز: چت آزاد + رأی‌گیری با حد نصاب منطقی\n' +
    '۴️⃣ دادگاه: متهم ۱۵ ثانیه فرصت دفاع دارد\n' +
    '۵️⃣ شب: چت ممنوع؛ هر پیام حذف می‌شود\n' +
    '۶️⃣ محقق و بازپرس می‌توانند شب، یک بازداشت با پنل محکوم/بخشش انجام دهند.\n' +
...
// ================== پیام‌ها (حذف پیام، نایت‌چت و کوپن) ==================
...
// پیام‌های معمولی: کوپن + حذف + نایت‌چت‌بلوک

bot.on('message', msg => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;

  if (msg.text && tryRedeemCoupon(msg)) {
    bot.deleteMessage(chatId, msg.message_id).catch(() => {});
    return;
  }
  ...
  // اگر داخل یکی از اتاق‌هاست و الان شب است، پیام را حذف کن
  for (const room of rooms.values()) {
    if (room.phase === 'NIGHT') {
      const inRoom = room.players.some(p => p.userId === userId);
      if (inRoom && msg.text) {
        bot.deleteMessage(chatId, msg.message_id).catch(() => {});
        bot.sendMessage(chatId, '🌙 الان فاز شب است؛ نمی_SPELLS.map(m =>
    `🔹 ${m.name}\nقیمت: ${m.price} سکه`
  ).join('\n\n');
}
function roleStoreKeyboard() {
