...
// پیام‌های معمولی: کوپن + چت با فرمت لیگ + نایت‌چت‌بلوک + مسدودی

bot.on('message', msg => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;

  // دستورات /... بالا هندل شدن، اینجا فقط پیام‌های معمولی
  ...
  // اگر در هیچ اتاقی نیست، فقط پیام رو پاک نکن (رفتار عادی تلگرام)
  if (!room) return;

  // اگر شب است، پیام را حذف کن و تذکر بده
  if (room.phase === 'NIGHT') {
: [], // { userId, nickname, role, alive, stats }
    day: 0,
    phase: null, // DAY / DAY_VOTE / COURT / NIGHT
    timers: [],
    votes: new Map(),
